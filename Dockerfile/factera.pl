...
if(exists($opts{o})) {$OUTPUTDIR = $opts{o};print $opt{o};}
...
if($VERBOSE == 10) {
    foreach my $gene (keys %breakpoints){
        ...
        foreach my $bp (sort {$breakpoints{$gene}->{$b} <=> $breakpoints{$gene}->{$a}} keys %{$breakpoints{$gene}}){
            $itor++;
print join("\t",'gene1',$gene,$breakpoints{$gene}->{$bp},$MAXBPS2EXAMINE),"\n";
            ...
            foreach my $gene2 (keys %{$pairs{$gene}}) {
                foreach my $bp2 (sort {$breakpoints{$gene2}->{$b} <=> $breakpoints{$gene2}->{$a}} keys %{$breakpoints{$gene2}}){
                    $itor2++;
print join("\t",'gene2',$gene2,$breakpoints{$gene2}->{$bp2},$MAXBPS2EXAMINE),"\n";
...
#print join("\t",$gene,$gene2,$bp2,$breakpoints{$gene2}->{$bp2}),"\n";
        }
    }
...
#print join("\t",$gene,$gene2,$bp2,$breakpoints{$gene2}->{$bp2}),"\n";                
 ...
 #               my $threshold = min(25,$cmpthresh*(min(length($read2), (length($read1))) - $k));
		my $threshold = min(10,$cmpthresh*(min(length($read2), (length($read1))) - $k));
#                my $thresholdb = min(25,$cmpthresh*(min(length($read2b), (length($read1b))) - $k));
		my $thresholdb = min(10,$cmpthresh*(min(length($read2b), (length($read1b))) - $k));
                my $doOffset = 1; #if 1, allow offset adjustment

#print join("\t",$read2, $k, $threshold, $read1, $doOffset, $clipOrder, $clipOrder2) ,"\n";
                my @same = compKmers(\%kmers1, \%kmers1rc, $read2, $k, $threshold, $read1, $doOffset, $clipOrder, $clipOrder2,$gene,$gene2);
print join("\t",$gene,'same',@same),"\n";
                my $clipOrderb = "NC";
                my $clipOrder2b = "NC";
                if($clipOrder eq "NC") {$clipOrderb = "CN";}
                if($clipOrder2 eq "NC") {$clipOrder2b = "CN";}
                my @sameb = compKmers(\%kmers1b, \%kmers1rcb, $read2b, $k, $thresholdb, $read1b, $doOffset, $clipOrderb, $clipOrder2b,$gene,$gene2);
print join("\t",$gene2,'sameb',@sameb),"\n";
...
print "$gene\_$gene2\_$bp\_$bp2\_$clipOrder\_$clipOrder2\n";
...
sub doBLAST{
    ...
    my $buffer = $_[3];
    my $min_len = $_[4];
print join("\t",$gene,$gene2,$buffer,$min_len),"\n"; 
...
`blastn -task 'megablast' -query $query -db $bdbname -outfmt 6 -num_threads $BLASTTHREADS -max_target_seqs 9999999 >>blast.result`;
	...
	my $tmp = $tokens[1]; ##read name include '_'
        my @temps=split/\_/,$tmp;
        $id = join(":",@temps[0..($#temps-1)]);
        my $readtype = $temps[-1];
...
#print "=============\n";
        ...
        my $left = $tokens[6];
        my $right = $tokens[7];
print join("\t",$id,$readtype,$percent,$percentsim,$align_len,$min_len),"\n";
...
#print join("\t",$gene1,$gene2,$sumk,$_[4],$clipsize-$k,$clip1,$clip2),"\n";
#print join("\t",$gene1,$gene2,$sumkrc,$_[4],($clipsize-$k),$clip1,$clip2),"\n";
